...
module.exports = async (client, message) => {
  ...
  const member = message.member,
    ...
    cooldownAmount = (commandFile.conf.cooldown || 1) * 250;
  ...
  if (!timestamps.has(member.id)) {
    ...
    if (now < expirationTime) {
      const timeLeft = (expirationTime - now) / 250;
      return message.channel.send(`Bu Komudu Tekrar Kullabilmek İçin **${timeLeft.toFixed(1)}** Saniye Kadar bekleyiniz`);
    }
